...
NSErrorDomain const SJFSMediaPlayerErrorDomain = @"SJFSMediaPlayerErrorDomain";

typedef struct {
    BOOL isFinished;
    FSMPMovieFinishReason reason;
} SJFSMediaPlaybackFinishedInfo;


@interface SJFSMediaPlayer ()
@property (nonatomic, strong) FSFFMoviePlayerController *player;
@property (nonatomic, strong, nullable) NSError *error;
@property (nonatomic) SJFSMediaPlaybackFinishedInfo playbackFinishedInfo;
...
@implementation SJFSMediaPlayer
...
#ifdef SJDEBUG
        [FSFFMoviePlayerController setLogReport:YES];
#else
        [FSFFMoviePlayerController setLogReport:NO];
        [FSFFMoviePlayerController setLogLevel:k_FS_LOG_INFO];
#endif
    });
}

- (instancetype)initWithURL:(NSURL *)URL startPosition:(NSTimeInterval)startPosition options:(nonnull FSOptions *)ops {
    self = [super init];
    if ( self ) {
        _volume = 1;
        _rate = 1;
        ...
        _player = [FSFFMoviePlayerController.alloc initWithContentURL:URL withOptions:ops];
        [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(_preparedToPlayDidChange:) name:FSMPMediaPlaybackIsPreparedToPlayDidChangeNotification object:_player];
        [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(_playbackDidFinish:) name:FSMPMoviePlayerPlaybackDidFinishNotification object:_player];
        [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(_playbackStateDidChange:) name:FSMPMoviePlayerPlaybackStateDidChangeNotification object:_player];
        [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(_loadStateDidChange:) name:FSMPMoviePlayerLoadStateDidChangeNotification object:_player];
        [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(_naturalSizeAvailable:) name:FSMPMovieNaturalSizeAvailableNotification object:_player];
        
        [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(_didSeekComplete:) name:FSMPMoviePlayerDidSeekCompleteNotification object:_player];
        [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(_seekRenderingStart:) name:FSMPMoviePlayerSeekAudioStartNotification object:_player];
        [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(_seekRenderingStart:) name:FSMPMoviePlayerSeekVideoStartNotification object:_player];

        [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(_firstVideoFrameRendered:) name:FSMPMoviePlayerFirstVideoFrameRenderedNotification object:_player];
...
- (void)replay {
    ...
    [self seekToTime:kCMTimeZero completionHandler:^(BOOL finished) {
        ...
        if ( !self ) return;
        if ( self.player.playbackState != FSMPMoviePlaybackStatePlaying )
            [self play];
...
- (nullable NSError *)error {
    return _playbackFinishedInfo.isFinished && _playbackFinishedInfo.reason == FSMPMovieFinishReasonPlaybackError ? _error : nil;
}
...
- (void)setVideoGravity:(SJVideoGravity)videoGravity {
    ...
    FSMPMovieScalingMode mode = FSMPMovieScalingModeAspectFit;
    if ( videoGravity == AVLayerVideoGravityResize )
        mode = FSMPMovieScalingModeFill;
    else if ( videoGravity == AVLayerVideoGravityResizeAspect )
        mode = FSMPMovieScalingModeAspectFit;
    else if ( videoGravity == AVLayerVideoGravityResizeAspectFill )
        mode = FSMPMovieScalingModeAspectFill;
...
- (void)_didSeekComplete:(NSNotification *)note {
    if ( [note.userInfo[FSMPMoviePlayerDidSeekCompleteErrorKey] boolValue] ) {
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)_toEvaluating {
    ...
    if ( _playbackFinishedInfo.isFinished && _playbackFinishedInfo.reason == FSMPMovieFinishReasonPlaybackError ) {
    ...
    if ( _playbackFinishedInfo.isFinished && self.timeControlStatus != SJPlaybackTimeControlStatusPaused ) {
        if ( _playbackFinishedInfo.reason == FSMPMovieFinishReasonPlaybackEnded ) {
    ...
    if ( self.timeControlStatus != SJPlaybackTimeControlStatusPaused ) {
        ...
        if ( _player.loadState & FSMPMovieLoadStateStalled ) {
            reason = SJWaitingToMinimizeStallsReason;
            status = SJPlaybackTimeControlStatusWaitingToPlay;
        }
        else if ( _player.loadState & FSMPMovieLoadStatePlayable ) {
            reason = nil;
    ...
    if ( self.assetStatus == SJAssetStatusReadyToPlay ) {
        if ( self.timeControlStatus != SJPlaybackTimeControlStatusPaused &&
            _player.playbackState == FSMPMoviePlaybackStatePaused ) {
            [_player play];
        }
    }
}
...
- (void)_updatePlaybackFinishedInfo:(NSNotification *)note {
    FSMPMovieFinishReason reason = [note.userInfo[FSMPMoviePlayerPlaybackDidFinishReasonUserInfoKey] integerValue];
    if ( reason == FSMPMovieFinishReasonPlaybackError ) {
        _error = [NSError errorWithDomain:SJFSMediaPlayerErrorDomain code:reason userInfo:@{
            @"fserror" : note
        }];
    }
